...
function IncreDecreCounter({ quantity, deleteOneCart, addOneCart,view, buyview }) {
  return (
    <div className={`flex items-center w-auto justify-evenly gap-2 py-2.5 px-5 me-2 mb-2 mt-1 ${quantity === 0 ? "hidden" : "flex"} ${view?"flex-col":""}`}>
      <NavLink to={'../checkout'}>
        <button className={`px-3 me-2 mb-2 mt-2 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-full border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 cursor-pointer py-1 ${buyview?"hidden":""}`}>Buy Now</button>
      </NavLink>
